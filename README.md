...
|方法名称|含义|能否接受回调函数|回调函数的作用|
|-|-|-|-|
|remoevAttr()|移除某个属性（只能移除一个）|否|空|
|addClass()|添加CSS类名（多个用空格分开）|能|调用该函数来计算需要设置的值|
|removeClass()|移除CSS类名（多个用空格分开）|能|调用该函数来计算需要设置的值|
|toggleClass()|添加或移除CSS类名（多个用空格分开）|能|回调函数返回true时表示设置，返回false表示移除|
|hasClass()|是否有CSS类名（只能判断一个）|否|空|
